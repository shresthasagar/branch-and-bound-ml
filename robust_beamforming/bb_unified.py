...
class BBenv(object):
    ...
    def reset(self, instance, max_ant,  oracle_opt=None, robust_beamforming=False, gamma=1, robust_margin=0.1):
...
def solve_bb(instance, max_ant=5, max_iter=10000, policy='default', policy_type='gnn',  oracle_opt=None, robust_beamforming=False):
    t1 = time.time()
    if policy_type == 'default':
        ...
        env = BBenv(observation_function=LinearObservation, epsilon=0.01)
    ...
    env.reset(instance, max_ant=max_ant, robust_beamforming=robust_beamforming)
    timestep = 0
    done = False
    lb_list = []
    ub_list = []
