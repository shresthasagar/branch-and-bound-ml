...
class ASBBenv(object):
    ...
    def reset(self, instance, max_ant,  oracle_opt=None):
        ...
        self.active_node = Node(z_mask=z_mask, z_sol=z, z_feas=self.z_incumbent, U=self.global_U, L=lower_bound, depth=1, node_index=self.node_index_count) 
    ...
    def is_optimal(self, node):
        if np.linalg.norm(node.z_sol - self.oracle_opt) < 0.0001:
            return True
        else:
            return False
...
def solve_bb_policy(instance, max_ant, max_iter=1000, policy='default', policy_type='gnn'):
    ...
    while timestep < max_iter and len(env.nodes)>0 and not done:
        print(timestep, env.global_U, env.global_L)
...
if __name__ == '__main__':
    ...
    # from enumeration import enumeration_antenna_selection
    # optimal_f = enumeration_antenna_selection(H, max_ant)
    # print('optimal solution', optimal_f)
    
    instance = np.stack((np.real(H), np.imag(H)), axis=0)
    global_U, _ = solve_bb_policy(instance, max_ant=max_ant, max_iter = 7000)
