...
import torch
from  acr_bb import  ACRBBenv, DefaultBranchingPolicy, RandomPolicy, solve_bb_policy
from observation import Observation, LinearObservation
from gnn_dataset import instance_generator
...
def collect_lto_samples():
    ...
    Path(valid_data_path).mkdir(exist_ok=True)

    M,N = 4,8
    instances = instance_generator(M,N)

    env = ACRBBenv(observation_function=Observation, epsilon=0.001)
    for i in range(3289,num_train):
        instance = next(instances)
        obs = env.reset(instance)[0]
        assert(isinstance(obs,Observation))
        w_opt = solve_bb_policy(instance)[0]
        data = [obs, w_opt, instance]
        filename = train_data_path + f'sample_{i}.pkl'
        
        with gzip.open(filename, 'wb') as f:
            pickle.dump(data, f)
        print('collected {} train samples'.format(i))

    for i in range(60,num_valid):
        instance = next(instances)
        obs = env.reset(instance)[0]
        w_opt = solve_bb_policy(instance)[0]
        data = [obs, w_opt, instance]
        ...
        with gzip.open(filename, 'wb') as f:
            pickle.dump(data, f)
        print('collected {} valid samples'.format(i))
