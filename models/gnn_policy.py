...
class GNNNodeSelectionPolicy(torch.nn.Module):
    ...
    def forward(self, obs, num_graphs):
        ...
        logits = logits.reshape([num_graphs, -1]).sum(dim=1)
...
class GNNLtOPolicy(torch.nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, constraint_features, edge_indices, edge_features, variable_features):
        ...
        # First step: linear embedding layers to a common dimension (64)
        constraint_features = torch.ones(constraint_features.shape)
        variable_features = torch.ones(variable_features.shape)
